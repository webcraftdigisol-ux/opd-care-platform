...
function fromPending(line: PendingLabLine): LineState {
  return {
    key: line.orderId,
...
export function LabCounterPage() {
  ...
  const { data: pendingLines, isFetching } = useQuery({
    queryKey: ['lab-pending', patient?.id],
    queryFn: () => getPendingLabLines(patient!.id),
    enabled: !!patient,
  });
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8">
      <h1 className="mb-6 text-2xl font-semibold text-teal">Lab Counter</h1>
      ...
      {patient && !receipt && (
        <div>
          ...
          {isFetching && <p className="text-gray-500">Loading ordered tests…</p>}
          ...
          <div className="space-y-3">
            ...
            {lines.length === 0 && !isFetching && (
              <p className="text-gray-500">No pending lab orders for this patient.</p>
            )}
          </div>
