...
import { createLabCatalogEntry, deleteLabCatalogEntry, listLabCatalog, updateLabCatalogEntry } from '../api/lab';
import type { LabTestCatalogEntry } from '@opd/shared';
...
export function LabCatalogPage() {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, name, price }: { id: string; name: string; price: number }) =>
      updateLabCatalogEntry(id, { name, price }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['lab-catalog'] });
      setEditingId(null);
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: deleteLabCatalogEntry,
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['lab-catalog'] }),
  ...
  function startEdit(entry: LabTestCatalogEntry) {
  ...
  function handleDelete(entry: LabTestCatalogEntry) {
    setDeleteError(null);
    if (!window.confirm(`Delete "${entry.name}" from the lab test catalog?`)) return;
      ...
      <div className="overflow-x-auto rounded-lg border border-gray-200 bg-white">
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            {catalog?.map((entry) =>
              editingId === entry.id ? (
                <tr key={entry.id} className="border-t border-gray-100 bg-teal-light/30" data-testid="lab-catalog-editing-row">
